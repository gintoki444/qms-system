...
const createWorksheet = (workbook, data, sheetIndex, totalSheets, nameCompany, globalIndex) => {
  ...
  worksheet.getCell('H37').value = 'CHECK BY………………………..…………….';
  worksheet.getCell('H37').font = { size: 12, bold: true, name: 'Browallia New' };

  // เพิ่มข้อความในแถวที่ H48-K48
  worksheet.mergeCells('H38:K38');
  worksheet.getCell('H38').value = 'FM-WH-05 Rev.02 : ' + moment(new Date()).format('DD/MM/YY');
  worksheet.getCell('H38').font = { size: 12, bold: true, name: 'Browallia New' };
  worksheet.getCell('H38').alignment = { horizontal: 'right' };
  ...
  worksheet.pageSetup.margins.bottom = 0.3;
};
...
const exportToExcel = async (data, nameCompany) => {
  ...
  for (let i = 0; i < totalSheets; i++) {
    const sheetData = data.slice(i * maxRowsPerSheet, (i + 1) * maxRowsPerSheet);
    createWorksheet(workbook, sheetData, i, Math.ceil(data.length / maxRowsPerSheet), nameCompany, globalIndex);
    globalIndex += sheetData.length;
  }
  // for (let i = 0; i < totalSheets; i++) {
