...
function HistoryPopup({ startDate, endDate, data, types, title, companyData, brandData }) {
            ...
            await adminRequest.getAllAuditLogs(sDate, eDate).then((response) => {
                ...
                if (types === 'reserves') {
                    ...
                    const dataWeight = response.filter(
                        (x) => (x.audit_system_id === id && x.audit_action === action && x.audit_system === "step1") ||
    ...
    const getWarhouseDate = (id) => {
        ...
        const numDate = proWare?.product_register_date
            ? ` (${calculateAge(proWare?.product_register_date)}) `
            : '-';
        ...
        <>
            <Dialog open={open} onClose={handleClose} aria-labelledby="responsive-dialog-title">
                <DialogTitle id="responsive-dialog-title" sx={{ backgroundColor: '#e5fffc' }}>
                    <Typography variant="h5" align="center">{txtTitle}</Typography>
                </DialogTitle>
                <DialogContent sx={{ maxWidth: { xs: 'auto', md: '25vw' }, mt: 2 }}>
                        ...
                        <Grid container spacing={1}>
                            <Grid item xs={12}>
                                <Typography variant="h5">
                                    บริษัท/ร้านค้า : {data.company}
                                </Typography>
                            </Grid>
                            <Grid item xs={12}>
                                <Typography variant="h5">
                                    วันที่เข้ารับสินค้า : {moment(data.pickup_date).format('DD/MM/YYYY')}
                                </Typography>
                            </Grid>
                            <Grid item xs={12}>
                                <Typography variant="h5">
                                    บริษัท (สินค้า) : {companyData?.product_company_name_th}
                                </Typography>
                            </Grid>
                            <Grid item xs={12}>
                    ...
                    {!loading && types === 'orders' && orderList.length > 0 ?
                        orderList.map((order, index) => (
                            <Grid container spacing={1} key={index} sx={{ mb: 2 }}>
                                <Grid item xs={12}>
                                    <Typography variant="h5">
                                        เลขที่คำสั่งซื้อ : {order.ref_order_id}
                                    </Typography>
                                </Grid>
                                <Grid item xs={12}>
                                    <Typography variant="h5">
                                        วันที่สั่งซื้อ : {moment(order.order_date.slice(0, 10)).format('DD/MM/YYYY')}
                                    </Typography>
                                </Grid>
                                <Grid item xs={12}>
                                    <Typography variant="h5">
                                        บริษัท (สินค้า) : {companyData.find((x) => x.product_company_id === order.product_company_id)?.product_company_name_th}
                                    </Typography>
                                </Grid>
                                <Grid item xs={12}>
                                    <Typography variant="h5">ตรา (สินค้า) : {brandData.find((x) => x.product_brand_id === order.product_brand_id)?.product_brand_name}</Typography>
                                </Grid>
                                <Grid item xs={12}>
                                    <Typography variant="h5">รายละเอียด : {order.description ? order.description : '-'}</Typography>
                                </Grid>
                                <Grid item xs={12}>
                                    <Typography variant="h5">เวลาที่กด : {auditlogList[index]?.audit_datetime ? auditlogList[index]?.audit_datetime?.slice(11, 19) + ' น.' : '-'}</Typography>
                                </Grid>
                                <Grid item xs={12}>
                                    <Typography variant="h5">Create By : {auditlogList[index]?.firstname ? auditlogList[index]?.firstname : '-'}</Typography>
                                </Grid>
                                <Grid item xs={12}>
                                    <Typography variant="h5">สูตรสินค้า</Typography>
                                    {order.items.length > 0 && order.items.map((items, oItems) => (
                            ...
                            <Grid container spacing={1} sx={{ mb: 2 }}>
                                <Grid item xs={12}>
                                    {orderList.map((order, index) => (
                                        <Grid container spacing={1} key={index} sx={{ mb: 2 }}>
                                            ...
                                            <Grid item xs={12}>
                                                <Typography variant="h5">
                                                    วันที่สั่งซื้อ : {order.ref_order_id}
                                                </Typography>
                                            </Grid>
                                            <Grid item xs={12}>
                                                <Typography variant="h5">
                                ...
                                <Grid item xs={12}>
                                    <Grid container spacing={1}>
                                        ...
                                        <Grid item xs={6} >
                                            <Typography variant="h5" align="right">
                                                <span style={{ color: 'red' }}>
                                                    {parseFloat(stepList[0]?.weight1).toFixed(3)}
                                                </span> (ตัน)
                                            </Typography>
                                        </Grid>
                                        <Grid item xs={12} >
                                        ...
                                        <Grid item xs={12} >
                                            <Typography variant="body">
                                                สถานีขึ้นชั่ง : {stepList[0]?.station_description ? 'สถานีที่ ' + stepList[0]?.station_description.replace(/ชั่งเบาที่ /g, '') : '-'}
                                            </Typography>
                                        </Grid>
                                        <Grid item xs={12} >
                                            <Typography variant="body">
                                ...
                                <Grid item xs={12}>
                                    <Grid container justifyContent="flex-end" spacing={1}>
                                        ...
                                        <Grid item xs={6} >
                                            <Typography variant="h5" align="right">
                                                <span style={{ color: 'red' }}>
                                                    {stepList[2]?.weight2 ? parseFloat(stepList[2]?.weight2).toFixed(3) : '-'}
                                                </span> (ตัน)
                                            </Typography>
                                        </Grid>
                                        <Grid item xs={12} >
                                        ...
                                        <Grid item xs={12} >
                                            <Typography variant="body">
                                                สถานีขึ้นชั่ง : {stepList[2]?.station_id !== 27 && stepList[2]?.station_description ? 'สถานีที่ ' + stepList[2]?.station_description.replace(/ชั่งหนักที่ /g, '') : '-'}
                                            </Typography>
                                        </Grid>
                                        <Grid item xs={12} >
                                            <Typography variant="body">
                                                Create By : {auditlogList[1]?.firstname ? auditlogList[1]?.firstname : '-'}
                                            </Typography>
                                        </Grid>
                                    </Grid>
                                </Grid>
                    ...
                    {!loading && types === 'teams' && (
                        <Grid container spacing={1}>
                            ...
                            <Grid item xs={12}>
                                <Typography variant="h5">
                                    โกดัง : {teamData?.team_warehouse_name ? teamData?.team_warehouse_name : '-'}
                                </Typography>
                            </Grid>
                            <Grid item xs={12}>
                                <Typography variant="h5">
                                    หัวจ่าย : {data.station_name ? data.station_name : '-'}
                                </Typography>
                            </Grid>
                            <Grid item xs={12}>
                            ...
                            <Grid item xs={12}>
                                <Typography variant="h5">
                                    พนักงานจ่ายสินค้า :  {teamData && teamData.team_checkers &&
                            ...
                            <Grid item xs={12}>
                                <Typography variant="h5">
                                    โฟล์คลิฟท์ :   {teamData && teamData.team_forklifts &&
                            ...
                            <Grid item xs={12}>
                                <Typography variant="h5">เวลาที่กด : {auditlogList?.audit_datetime ? auditlogList?.audit_datetime?.slice(11, 19) + ' น.' : '-'}</Typography>
                            </Grid>
                            <Grid item xs={12}>
                                        ...
                                        <Grid item xs={12}>
                                            {order.items.length > 0 && order.items.map((items, oItems) => (
                                                <Grid container justifyContent="flex-end" spacing={1} key={oItems}>
                                                    ...
                                                    <Grid item xs={12}>
                                                        <Typography variant="h5">
                                                            ข้อมูลกองสินค้า : {items.name ? items.name : '-'}
                                                        </Typography>
                                                    </Grid>
                                                    <Grid item xs={12}>
                                                        <Typography variant="h5">จำนวน : {items?.quantity ? items?.quantity : '-'}
                                                        </Typography>
                                                    </Grid>
                                                    <Grid item xs={12}>
                                                        <Typography variant="h5">โกดัง : {getWarhouseDate(items.product_register_id)
                                                            // product_register_id
                                                            // items.name ? items.name : '-'
                                                        }
                                                        </Typography>
                                                    </Grid>
                                                    {items.smash_quantity !== '0.0000' && items.sling_hook_quantity !== '0.0000' && items.sling_sort_quantity !== '0.0000' && items.jumbo_hook_quantity !== '0.0000' && (
                                                        <Grid item xs={12}>
                                                            <Typography variant="h5">
                                                                ประเภท :
                                                                    ...
                                                                    {items.smash_quantity !== '0.0000' && (
                                                                        <>
                                                                            ...
                                                                            <Grid item xs={6} >
                                                                                <Typography variant="h5" align="right">
                                                                                    <span style={{ color: 'red' }}>
                                                                                        {parseFloat((items.smash_quantity * 1)).toFixed(3)}
                                                                                    </span> (ตัน)
                                                                                </Typography>
                                                                            </Grid>
                                                                        </>
                                                                    ...
                                                                    {items.sling_hook_quantity !== '0.0000' && (
                                                                        <>
                                                                            ...
                                                                            <Grid item xs={6} >
                                                                                <Typography variant="h5" align="right">
                                                                                    <span style={{ color: 'red' }}>
                                                                                        {parseFloat((items.sling_hook_quantity * 1)).toFixed(3)}
                                                                                    </span> (ตัน)
                                                                                </Typography>
                                                                            </Grid>
                                                                        </>
                                                                    ...
                                                                    {items.sling_sort_quantity !== '0.0000' && (
                                                                        <>
                                                                            ...
                                                                            <Grid item xs={6} >
                                                                                <Typography variant="h5" align="right">
                                                                                    <span style={{ color: 'red' }}>
                                                                                        {parseFloat((items.sling_sort_quantity * 1)).toFixed(3)}
                                                                                    </span> (ตัน)
                                                                                </Typography>
                                                                            </Grid>
                                                                        </>
                                                                    ...
                                                                    {items.jumbo_hook_quantity !== '0.0000' && (
                                                                        <>
                                                                            ...
                                                                            <Grid item xs={6} >
                                                                                <Typography variant="h5" align="right">
                                                                                    <span style={{ color: 'red' }}>
                                                                                        {parseFloat((items.jumbo_hook_quantity * 1)).toFixed(3)}
                                                                                    </span> (ตัน)
                                                                                </Typography>
                                                                            </Grid>
                                                                        </>
